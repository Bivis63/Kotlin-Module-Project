...
class Menu {
    ...
    private fun <T : Storage> screen(value: MutableList<T>, textValue: List<String>) {

        if (textValue[0] != "заметке") {
            println("Список ${textValue[0]}:")
            if (value.size >= 1) {
                for (i in value.indices) {
                    if (i == 0) {
                        println("$i. Создать ${textValue[1]}")
                    }
                ...
                println("0. Создать ${textValue[1]}")
                println("1. Выход")
            }
        } else {
            println("0. Показать заметку")
            println("1. Добавить текст к ${textValue[0]}")
            println("2. Выход")
        }
    }
